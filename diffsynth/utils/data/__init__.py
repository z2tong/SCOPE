"""Video and image I/O utilities for loading, saving, and preprocessing.

Provides memory-efficient video/image reading, center-crop-and-resize,
and video writing with optional audio merging.
"""
...
class LowMemoryVideo:
    ...
    Args:
        file_name: Path to a video file readable by imageio.
    """
    ...
    def __getitem__(self, item: int) -> Image.Image:
        return Image.fromarray(
            np.array(self.reader.get_data(item))
        ).convert("RGB")
...
def crop_and_resize(image: Image.Image, height: int, width: int) -> Image.Image:
    """Center-crops and resizes an image to the target dimensions.

    Crops the image to match the target aspect ratio (center crop), then
    resizes to exact (width, height).
    ...
    Returns:
        Resized PIL Image with exact (width, height) dimensions.
    """
...
def save_video(
    ...
    Args:
        frames: List of PIL Images (one per frame).
        save_path: Output file path (e.g., ``"output.mp4"``).
        fps: Frames per second.
        quality: Video quality (1-10, higher is better).
        ffmpeg_params: Optional extra ffmpeg parameters.
    """
...
def merge_video_audio(video_path: str, audio_path: str) -> None:
    ...
    The output duration is set to the shorter of the two streams.
    The merged result overwrites the original video file.
